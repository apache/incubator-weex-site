...
## 子组件
和 [`<list>`](/zh/docs/list.html)组件一样, `<waterfall>` 组件的子组件只能包括以下四种组件或是 `fix` 定位的组件，其他形式的组件将不能被正确渲染。
* [`<cell>`](/zh/docs/cell.html)：用于定义列表中的子列表项，类似于 HTML 中的 ul 之于 li。Weex 会对 `<cell>` 进行高效的内存回收以达到更好的性能。
* `<header>`：当 `<header>` 到达屏幕顶部时，吸附在屏幕顶部。
* [`<refresh>`](/zh/docs/refresh.html)：用于给列表添加下拉刷新的功能。
* [`<loading>`](/zh/docs/loading.html)：`<loading>` 用法与特性和 `<refresh>` 类似，用于给列表添加上拉加载更多的功能。
...
其他支持的属性参见 [List 组件属性部分](/zh/docs/list.html#%E5%B1%9E%E6%80%A7)
