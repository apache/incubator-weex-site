Weex 初步接入请参考：[https://github.com/weexteam/article/issues/25](https://github.com/weexteam/article/issues/25)
...
第一种图片适配器是必须实现，第二种图片适配器是可选实现。Weex 对于这两种图片适配器均没有默认实现。
